...
app.get ('/api/employees/:employeeId', async (c) => {
...
//管理者用：有給残数マスター一覧取得
app.get('/api/admin/balances', adminGuard, async (c) => {
  const { data, error } = await supabase
    .from('leave_balances')
    .select('employee_id, employee_name, paid_given, paid_used')
 ...
 */
app.post('/api/admin/check-grant', adminGuard, async (c) => {
  // 🕒 今日の日付（JST前提でざっくり）
  const now = new Date();
  const yyyy = now.getFullYear();
  const mm = String(now.getMonth() + 1).padStart(2, '0');
...
app.get('/test/check-grant', async (c) => {
  // 🕒 今日の日付（JST前提でざっくり）
  const now = new Date();
  const yyyy = now.getFullYear();
  const mm = String(now.getMonth() + 1).padStart(2, '0');
...
//管理者用：有給残数マスタの更新
app.put('/api/admin/balances/:employeeId', adminGuard, async (c) => {
...
app.post('/api/admin/approve', adminGuard, async (c) => {
  // 承認処理
});

app.post('/api/admin/reject', adminGuard, async (c) => {
  // 却下処理
});

app.get('/api/admin/csv', adminGuard, async (c) => {
  // CSV処理
});
...
// 管理者用：有給マスタ更新
app.patch('/api/admin/balances/:employeeId', adminGuard, async (c) => {
  const employeeId = c.req.param('employeeId');
...
app.post('/api/admin/remind-balances', adminGuard, async (c) => {
  const supabase = createClient(
    process.env.SUPABASE_URL!,
...
//管理者用：全従業員の申請一覧
app.get('/api/admin/leaves', adminGuard, async (c) => {
  const employeeId = c.req.query('employeeId');
...
// 管理者用：ステータス更新
app.patch('/api/admin/leaves/:id/status', adminGuard, async (c) => {
...
console.log(`Hono running: http://localhost:${port}`);
